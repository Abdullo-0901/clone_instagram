...
const Publ = (id ?:PropsUserStories) => {
 const { data : post } = UseGetPost() ;
  ...
  return (
    <div className='flex flex-wrap m-auto w-full my-[80px] items-center'>
    {
            ...
            return ( 
               ...
               >
                  {userPost.images.map((elem,id)=>{
                     ...
                     <SwiperSlide key={id}>
                      {  elem.slice(-3) == "MP4" ?  
                      <video controls src={`${import.meta.env.VITE_APP_FILES_URL}${elem}`}className="w-[100%] h-[100%] object-cover" />:
                      <img  src={`${import.meta.env.VITE_APP_FILES_URL}${elem}`}className="w-[100%] h-[100%] object-cover" />
                      }
                    </SwiperSlide>
                 )
               })}
